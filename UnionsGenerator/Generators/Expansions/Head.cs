...
internal sealed class Head(TargetDataModel model)
    : ExpansionBase(model, Macro.Head)
{
    ...
    public static void ContainingClassHead(ExpandingMacroBuilder builder, ITypeSymbol nestedType)
    {
        ...
        static IEnumerable<ITypeSymbol> getContainingTypes(ITypeSymbol symbol)
        {
            return symbol.ContainingType != null
                ? getContainingTypes(symbol.ContainingType).Append(symbol.ContainingType)
                : (IEnumerable<ITypeSymbol>)Array.Empty<ITypeSymbol>();
        }
    }
    protected override void Expand(ExpandingMacroBuilder builder)
    {
        ...
        //    {
        //        _ = builder /
        //            "[System.Text.Json.Serialization.JsonConverter(typeof(" * Model.Symbol.ToMinimalOpenString() * ".JsonConverter))]";
        //    }

        //    if(Model.Annotations.Settings.Layout == LayoutSetting.Small && !Model.Symbol.IsGenericType)
        //    {
        //        _ = builder /
        //            String.Empty % "[System.Runtime.InteropServices.StructLayout(System.Runtime.InteropServices.LayoutKind.Sequential, Pack = 1)]";
        //    }
        //}
