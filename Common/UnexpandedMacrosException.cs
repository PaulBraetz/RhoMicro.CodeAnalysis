...
sealed class UnexpandedMacrosException<TMacro>(IReadOnlyList<TMacro> macros) 
    : Exception("Unable to build string; there is at least one unexpanded macro present in the builder. " +
                $"Make sure to provide expansions for all macros in use. Unexpanded macros: {String.Join(", ", macros)}")
{
    /// <summary>
